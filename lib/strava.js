...
function newStravaSession(req, res, token)
{
	var newSession = {};

	if(req.session.stravaToken)
		req.session.stravaToken = token;
...
function getStravaSession(req, res)
{
	...
	if(!req.sessionData.strava)
		newStravaSession(req, res, req.session.stravaToken);
...
stravaEx.prototype.useAthlete = function(req, res, next)
{
	if(!req.session)
		next();
	...
	if(!stravaSession.stravaToken && req.session.stravaToken)
	{
		stravaSession.stravaToken = req.session.stravaToken;
	}
	else if(stravaSession.stravaToken && !req.session.stravaToken)
	{
		req.session.stravaToken = stravaSession.stravaToken;
	}
	...
	if(stravaSession.stravaToken && !stravaSession.athlete)
	{
		strava.athlete.get({'access_token':stravaSession.stravaToken},function(err,payload) {
        	...
        	{
        		req.session.stravaToken = undefined;
        		next();
        	}
		});
	}
...
stravaEx.prototype.useAccount = function(req, res, next)
{
	if(!req.session)
		next();
	...
	if(stravaSession.stravaToken && !stravaSession.account)
	{
		if(stravaSession.athlete)
		{
			accounts.getOrCreateAccountFromStrava(stravaSession.athlete, function(err, account)
			{
				if(!err)
				{
...
function sendActivitiesResult(req, res, stravaSession, page, per_page)
{
	var start = (page-1) * per_page;

	stravaSession.activitiesPage = [];

	while(start >= stravaSession.activities.length)
	{
		page--;
		start -= per_page;
	}

	for(var i=start;i<stravaSession.activities.length && i<start+per_page;i++)
	{
		stravaSession.activitiesPage.push(stravaSession.activities[i]);
	}
...
stravaEx.prototype.getLists = function (req, res)
{
	...
	var urlParsed = url.parse(req.url, true);
	var pageNum = 1;
	var forcedRefresh = (!stravaSession.activities || stravaSession.lastGetList < Date.now() - 3600000);

	if(urlParsed && urlParsed.query)
	{
		pageNum = urlParsed.query.page || 1;
		if(urlParsed.query.refresh)
			forcedRefresh = true;
	}
	...
	{
		if(stravaSession.activities && !forcedRefresh)
		{
			sendActivitiesResult(req, res, stravaSession, pageNum, 20);
		}
		else
		{
			strava.athlete.listActivities({'access_token':stravaSession.stravaToken, page: 1, per_page: 200},function(err,payload) {
	        	if(!err)
	        	{
	        		stravaSession.activities = payload;
	        		stravaSession.lastGetList = Date.now();
	        		sendActivitiesResult(req, res, stravaSession, pageNum, 20);
	        	}
	        	else
	        	{

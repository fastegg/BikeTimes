...
stravaEx.prototype.getLists = function (req, res)
{
	...
	{
        strava.athlete.listActivities({'access_token':stravaSession.stravaToken},function(err,payload) {
        	if(!err)
        	{
...
stravaEx.prototype.loadStream = function(req, res)
{
	...
	if(urlParsed && urlParsed.query && urlParsed.query.id);
	{
		...
		if(activity)
		{
			strava.streams.activity({access_token: stravaSession.stravaToken, id: urlParsed.query.id, types: 'latlang,time,distance,altitude'}, function(err, payload) {
				if(!err)
				{
					var stravaSession = getStravaSession(req, res);
					var i;
					...
					stravaSession.acitivity.streams = {};
					
					for(i=0;i<payload.length;i++)
					{
						stravaSession.activity.streams[payload.type] = payload.data;
					}

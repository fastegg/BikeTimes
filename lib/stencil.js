...
function map(val, choices, returnOrig)
{
...
{
	return map(this,choices,returnOrig);
}

Number.prototype.map = function(choices, returnOrig)
{
	return map(this.toString(),choices,returnOrig);
}
...
Number.prototype.MtoMI = function()
{
	return this / 1609.34;
}

Number.prototype.MPStoMPH = function()
{
	return this * 2.23694;
}
...
Number.prototype.MPStoKPH = function()
{
	return this * 3.6;
}
...
function parseRecurse($, curPath)
{
	...
	if($.type === 'tag' && $.attribs)
	{
		for(var attrib in $.attribs)
		{
			...
			else if(attrib === 'data-run')
			{
				$.datarun = evaluatePath($.attribs[attrib]);
			}
			else if(attrib === 'data-dev')
			{
				if(process.env.isProduction)
					$.dataif = "0";
			}
			else if(attrib === 'data-prod')
			{
				if(!process.env.isProduction)
					$.dataif = "0";
			}
...
function fillRecurse($, obj, objType, root, vars)
{
	...
	if($.datarun)
	{
		evalArgs(obj, root, $.datarun, vars);
	}
...
stencil.prototype.fillStencilWithReq = function(stencilName, req, optionalObj)
{
	var rootObj = {};
	var vars = {index: -1, urlVars: {}};

	rootObj.req = req;
	rootObj.obj = optionalObj;

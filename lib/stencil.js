...
function evalDataIf(obj, root, args)
{
	var rtn = evalArgs(obj, root, args)
...
function fillRecurse($, obj, objType, root)
{
	...
	else if($.type === 'tag')
	{
		...
		else if(objType === '[object Object]')
		{
			for(i in $.children)
			{
				addChildren(objReturn.children,fillRecurse($.children[i], obj, objType, root));
			}
		}
		else if(objType === '[object Array]')
		{
			...
			for(i=0;i<obj.length; i++)
			{
				...
				for(n in $.children)
				{
					...
					var newChild = fillRecurse($.children[n], obj[i], newType, root);

					if(newChild)
						addChildren(newRtnObj.children,newChild);
				}
	...
	else if($.type === 'root')
	{
		for(n in $.children)
		{
			addChildren(objReturn.children, fillRecurse($.children[n], obj, objType, root));
		}
	}
	else if($.type === 'script')
	{
...
stencil.prototype.fillStencilWithReq = function(stencilName, req)
{
	...
	return this.fillStencil(stencilName, rootObj);
}
